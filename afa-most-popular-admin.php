		...
		<form method="post" action="options.php">
			...
			<table class="form-table">
				...
				<tr valign="top">
					...
					<td>
						<?php
						$selected_post_types = get_option( 'afa_display_post_types', array( 'post' ) );
						$all_post_types = get_post_types( array( 'public' => true ), 'objects' );
						foreach ( $all_post_types as $post_type ) {
							$checked = checked( in_array( $post_type->name, $selected_post_types ) );
							$value = esc_attr( $post_type->name );
							$title = esc_html( $post_type->labels->name );
							echo "<label><input type='checkbox' name='afa_display_post_types[]' value='{$value}' {$checked}>{$title}</label><br/>";
						}
						?>
					</td>
				</tr>
